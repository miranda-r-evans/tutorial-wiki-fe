describe('/tutorial/create', () => {
  beforeEach(() => {
    ...
    cy.intercept('POST', 'tutorial', { statusCode: 201, body: { rootId: 1 } })
  ...
  it('should accept input text, create a new section, and embed a section', () => {
    cy.get('.heading-input input').should('have.attr', 'placeholder', 'Type a heading')
      .and('have.value', '')
    cy.get('.heading-input input').type('test heading')
    cy.get('.heading-input input').should('have.value', 'test heading')

    cy.get('.mce-content-body').should('have.attr', 'aria-placeholder', 'Write your tutorial')
  ...
  it('should be able to nest sections and display a text editing area above and below the section', () => {
    ...
    cy.realPress('Enter')
    cy.get('.tutorial:first p:first').should('have.text', 'paragraph layer 1')
  ...
  it('should insert the new section after the line that the cursor is on', () => {
    ...
    cy.get('.mce-content-body:first').as('first').find('p:first').should('have.text', 'a')
  ...
  it('should redirect to a tutorial page upon saving', () => {
    ...
    cy.url().should('eq', 'http://localhost:3000/tutorial/1');
  })
})
